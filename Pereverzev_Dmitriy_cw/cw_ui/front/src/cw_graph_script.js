...
function form() {
    ...
    for (let i = 0; i < data_now.average_case.length; i++)
        arr1[i] = {
            ...
            "time": data_now.average_case[i].time,
    ...
    for (let i = 0; i < data_now.worst_case.length; i++)
        arr2[i] = {
            ...
            "time": data_now.worst_case[i].time,
...
var margin = { top: 10, right: document.getElementById("my_dataviz").offsetWidth * 0.05, bottom: 30, left: document.getElementById("my_dataviz").offsetWidth * 0.05 },
    width = document.getElementById("my_dataviz").offsetWidth * 0.4 - margin.left - margin.right,
...
function rebuild(id) {
    ...
    svg = d3.select(document.getElementById("my_dataviz"))
        .append("svg")
        .attr("id", `graph_${id}`)
        .attr("class", `graph graph_${id == 1 ? 'left' : 'right'}`)
    ...
    var line = d3.line()
        ...
        .y(function (d) { return y(+id == 1 ? d.time : d.steps) })
    ...
    svg
        ...
        .attr("cy", function (d) { return y(id == 1 ? d.time : d.steps) })
        .attr("r", 5)
        .attr("stroke", "white")
    ...
    svg
        ...
        .attr("transform", function (d) { return "translate(" + x(d.value.num) + "," + y(id == 1 ? d.value.time : d.value.steps) + ")"; })
        .attr("x", 12)
...
setTimeout((a, b) => { rebuild(1); rebuild(2); }, 500, 1, 2);
